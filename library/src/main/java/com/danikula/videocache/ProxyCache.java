...
class ProxyCache {
    ...
    public int read(byte[] buffer, long offset, int length) throws ProxyCacheException {
        ...
        if (cache.isCompleted() && percentsAvailable != 100) {//缓存完成
    ...
    public void shutdown() {
        synchronized (stopLock) {
            MyLog.d(LOG_TAG, "Shutdown proxy for " + source);
            try {
                stopped = true;
    ...
    protected final void onError(final Throwable e) {
        ...
        if (interruption) {
            MyLog.d(LOG_TAG, "ProxyCache is interrupted");
        } else {
            MyLog.e(LOG_TAG, "ProxyCache error", e);
        }
    }
